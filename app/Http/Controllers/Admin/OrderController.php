...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($q) {
            $orders = $orders->where('code', 'like', '%' . $q . '%')
                ->orWhere('customer_admin_name', 'like', '%' . $q . '%')
    ...
    public function cancel($id)
    {
        $order = Order::where('id', $id)
            ->whereIn('status', [Order::PENDING, Order::DIPROSES])
            ->firstOrFail();
    ...
    public function update(OrderRequest $request, $id)
    {
        ...
        $saved = false;
        $saved = DB::transaction(function () use ($order, $params) {
            $order->update($params);
        ...
        if ($saved) {
            ...
            Sesion::flash('error', 'Order gagal disimpan');
        }
